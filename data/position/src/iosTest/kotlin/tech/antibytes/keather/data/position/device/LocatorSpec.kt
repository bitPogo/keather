...
import tech.antibytes.keather.data.position.locator.AppleLocationContractProtocol
...
class LocatorSpec {
    ...
    fun `Given getCurrentLocation is called it propagates an error if no result was returned`() = runTest {
        ...
        // When
        val result = Locator(appleLocator).fetchPosition()
    ...
    fun `It fulfils Locator`() {
        Locator(AppleLocatorContractProtocolMock()) fulfils PositionRepositoryContract.Locator::class
    }
}
